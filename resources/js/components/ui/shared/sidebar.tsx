...
export default function Sidebar({
    ...
    const isItemSelected = (href: string) => {
        ...
        return (
            normCurrent === normPath || normCurrent.startsWith(normPath + "/")
        );
    };
    ...
    return (
        <>
                        ...
                        {navItems.map((item, index) => (
                            <li key={index} className="group">
                                {item.subItems ? (
                                    <>
                                        ...
                                        >
                                            <div className="flex items-center gap-4">
                                                <span className="text-xl text-purple-500 group-hover:text-purple-600 transition-colors duration-300">
                                                    {item.icon}
                                                </span>
                                        ...
                                        <ul
                                            className={`ml-10 mt-1 space-y-1 overflow-hidden transition-all duration-300 ease-in-out ${openMenus[index] ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}
                                        >
                                            {item.subItems.map(
                                                (subItem, subIndex) => (
                                                    <li key={subIndex}>
                                                        ...
                                                        >
                                                            <div
                                                                className={`w-1 h-1 rounded-full mr-2 ${isItemSelected(subItem.href) ? "bg-purple-500" : "bg-gray-300"}`}
                                                            />
                                                            <span>
                                                                {subItem.label}
                                                            </span>
                                    ...
                                    >
                                        <span
                                            className={`text-xl transition-colors duration-300 ${isItemSelected(item.href || "") ? "text-purple-600" : "text-purple-500 group-hover:text-purple-600"}`}
                                        >
                                            {item.icon}
                                        </span>
                                        <span
                                            className={`text-base ${isItemSelected(item.href || "") ? "font-medium" : ""}`}
                                        >
                                            {item.label}
                                        </span>
                                        {isItemSelected(item.href || "") && (
            ...
            >
                ...
                {/* Logo */}
                <div className="p-8 flex justify-center">
                    <img src="/logol.svg" alt="Logo" className="w-40 h-auto" />
                </div>
